package com.science09.cnblogs.dal;
...
import com.science09.cnblogs.model.Comment;
...
public class CommentDalHelper {
	...
	private boolean Exist(int commentId, Comment.EnumCommentType commentType) {
		...
		String[] args = { String.valueOf(commentId),
				String.valueOf(commentType.ordinal()) };
		Cursor cursor = db.query(Config.DB_COMMENT_TABLE, null, where, args,
				null, null, null);
	 ...
	 * 分页
	 */
	public List<Comment> GetCommentListByPage(int pageIndex, int pageSize,
			int contentId, Comment.EnumCommentType commentType) {
	 ...
	 * @param where
	 */
	public List<Comment> GetCommentListByWhere(String limit, String where,
			String[] args) {
		...
		while (cursor != null && cursor.moveToNext()) {
			...
			entity.SetPostUserName(cursor.getString(cursor
					.getColumnIndex("PostUserName")));
			entity.SetPostUserUrl(cursor.getString(cursor
					.getColumnIndex("PostUserUrl")));
			entity.SetContentId(cursor.getInt(cursor
	...
	public void SynchronyData2DB(List<Comment> commentList) {
		...
		for (int i = 0, len = commentList.size(); i < len; i++) {
			...
			contentValues.put("PostUserUrl", commentList.get(i)
					.GetPostUserUrl());
			contentValues.put("PostUserName", commentList.get(i)
					.GetPostUserName());
			String content = "";
		...
		synchronized (_writeLock) {
			...
			try {
				for (int i = 0, len = list.size(); i < len; i++) {
					boolean isExist = Exist(commentList.get(i).GetCommentId(),
							commentList.get(i).GetCommentType());
					if (!isExist) {
