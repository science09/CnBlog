...
import com.science09.cnblogs.enums.EnumResultType;
import com.science09.cnblogs.model.FavList;
...
public class FavDalHelper {
	...
	private boolean Exist(int contentId, FavList.EnumContentType contentType) {
		...
		String[] args = { String.valueOf(contentId),
				String.valueOf(contentType.ordinal()) };
		Cursor cursor = db.query(Config.DB_FAV_TABLE, null, where, args, null,
				null, null);
	 ...
	 * 分页
	 */
	public List<FavList> GetFavListByPage(int pageIndex, int pageSize,
			FavList.EnumContentType contentType) {
	 ...
	 * 得到对象
	 */
	public FavList GetFavEntity(int contentId,
			FavList.EnumContentType contentType) {
		String limit = "1";
		String where = "ContentId=? and ContentType=?";
		String[] args = { String.valueOf(contentId),
	 ...
	 * @param where
	 */
	public List<FavList> GetFavListByWhere(String limit, String where,
			String[] args) {
		...
		while (cursor != null && cursor.moveToNext()) {
			...
			entity.SetContentType(FavList.EnumContentType.values()[cursor
					.getInt(cursor.getColumnIndex("ContentType"))]);
	 ...
	 * @param list
	 */
	public EnumResultType.EnumActionResultType SynchronyData2DB(
		...
		synchronized (_writeLock) {
			...
			try {
				for (int i = 0, len = list.size(); i < len; i++) {
					boolean isExist = Exist(favList.get(i).GetContentId(),
							favList.get(i).GetContentType());
					if (isExist) {
