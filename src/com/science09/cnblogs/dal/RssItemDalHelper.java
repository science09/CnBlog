...
import com.science09.cnblogs.model.RssItem;
...
public class RssItemDalHelper {
	...
	private boolean Exist(String link) {
		String where = "Link=?";
		String[] args = { link };
		Cursor cursor = db.query(Config.DB_RSSITEM_TABLE, null, where, args,
				null, null, null);
	 ...
	 * @param where
	 */
	public List<RssItem> GetRssItemsByWhere(String limit, String where,
			String[] args) {
		...
		while (cursor != null && cursor.moveToNext()) {
			...
			entity.SetDescription(cursor.getString(cursor
			...
			entity.SetLink(link);
			entity.SetIsReaded(Boolean.getBoolean(cursor.getString(cursor
					.getColumnIndex("IsReaded"))));
			entity.SetAuthor(cursor.getString(cursor.getColumnIndex("Author")));
			boolean isDigg = cursor.getString(cursor.getColumnIndex("IsDigg"))
					.equals("1");
