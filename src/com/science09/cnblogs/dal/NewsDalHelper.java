...
public class NewsDalHelper {
	...
	private boolean Exist(int newsId) {
		...
		Cursor cursor = db.query(Config.DB_NEWS_TABLE, null, where, null, null,
				null, null);
	 ...
	 * @param where
	 */
	public List<News> GetNewsListByWhere(String limit, String where,
			String[] args) {
	...
	public void SynchronyData2DB(List<News> newsList) {
		...
		for (int i = 0, len = newsList.size(); i < len; i++) {
			...
			contentValues.put("Published",
					AppUtil.ParseDateToString(newsList.get(i).GetAddTime()));
			...
			try {
				...
				for (int i = 0, len = list.size(); i < len; i++) {
					...
					if (!isExist) {
						...
						SynchronyContent2DB(list.get(i).getAsInteger("NewsId"),
