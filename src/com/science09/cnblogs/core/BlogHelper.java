package com.science09.cnblogs.core;
...
import com.science09.cnblogs.parser.BlogListXmlParser;
import com.science09.cnblogs.parser.BlogXmlParser;
...
public class BlogHelper extends DefaultHandler {
	...
	public static ArrayList<Blog> GetBlogList(int pageIndex) {
		...
		Log.d("BlogHelper", url);
		String dataString = NetUtils.GetContentFromUrl(url);
		if(dataString == null) {
			return null;	
		} else {
	...
	public static String GetBlogContentByIdWithNet(int blogId) {
		...
		String xml = NetUtils.GetContentFromUrl(url);
		if (xml == "") {
			return "";
		}
	...
	public static String GetBlogById(int blogId, Context context) {
		...
		if (null == entity || entity.GetBlogContent().equals("")) {
			 ...
			 * .EnumImageType.Blog, blogContent); if
			 * (Config.IS_SYNCH2DB_AFTER_READ) {
	 ...
	 * @return
	 */
	public static String ParseBlogString(String dataString) {
		SAXParserFactory saxParserFactory = SAXParserFactory.newInstance();
		String blogContent = "";
		try {
			XMLReader xmlReader = saxParserFactory.newSAXParser()
