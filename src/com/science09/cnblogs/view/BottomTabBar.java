package com.science09.cnblogs.view;
...
public class BottomTabBar extends LinearLayout implements View.OnClickListener,
	...
	private static final int HEIGHT_NAVIGATION_BAR = 40; // 导航栏默认高度，不包括指示器高度，单位是dp
	private static final int HEIGHT_INDICATOR = 3; // 指示器默认高度，单位是dp
	private int darkGreen = android.R.color.holo_green_dark;
	...
	public void attachToParent(ViewGroup viewGroup, String[] titles,
		...
		mLine.setImageResource(android.R.color.holo_green_dark);
		mLine.setScaleType(ScaleType.MATRIX);
		mLine.setLayoutParams(new LinearLayout.LayoutParams(
				android.view.ViewGroup.LayoutParams.MATCH_PARENT, dip2px(1)));
		this.addView(mLine);
		...
		mIndicater.setImageResource(android.R.color.holo_green_dark);
		...
		ll_bottom.setLayoutParams(new LayoutParams(
				android.view.ViewGroup.LayoutParams.MATCH_PARENT,
				dip2px(HEIGHT_NAVIGATION_BAR)));
		ll_bottom.setBackgroundColor(getResources().getColor(
				android.R.color.white));
		...
		positions[0] = 0;
		mList.get(positions[0])
				.setTextColor(getResources().getColor(darkGreen));
		int distance = getScreenWidth(context) / titleCount;
